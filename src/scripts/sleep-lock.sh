#!/bin/bash
#
# Locks before suspend and waits for the session lock to become secure.
#
# Ported from Omarchy 4's bin/omarchy-system-sleep-lock (MIT, Copyright David
# Heinemeier Hansson), with the shell IPC calls pointed at Ghost.
#
...
lock_ipc() {
    ...
    timeout --kill-after=0.1s "$seconds" qs -p "$GHOST_PATH" ipc call "$@"
}
...
report_unsecured() {
    printf 'ghost sleep-lock: suspending without a secure lock (%s)\n' "$1" >&2

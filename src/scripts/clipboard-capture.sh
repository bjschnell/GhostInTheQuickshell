...
#
# Ported from Omarchy 4's shell/plugins/clipboard/capture.sh (MIT, Copyright
# David Heinemeier Hansson), with the image store moved under ~/.cache/ghost.
#
# The two guards below are the point of this script: CLIPBOARD_STATE=sensitive
# is what wl-paste sets for a selection the source marked as such, and
# x-kde-passwordManagerHint is what password managers advertise. Either one
# means the payload is never read, never hashed, and never written anywhere.

set -o pipefail

CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/ghost"
IMAGE_DIR="$CACHE_DIR/clipboard-images"
mkdir -p "$IMAGE_DIR"

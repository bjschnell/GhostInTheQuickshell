...
// KanbanBoard — three columns, JSON at $HOME/.config/Ghost/src/user_data/tasks.json.
//
// Key behaviours:
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: function(line) {
                var h = line.trim()
                if (h === "") return
                root._filePath = h + "/.config/Ghost/src/user_data/tasks.json"
                mkProc.command = [
                    "bash", "-c",
                    "[ -f '" + root._filePath + "' ] || " +
                    "(mkdir -p \"$HOME/.config/Ghost/src/user_data\" && " +

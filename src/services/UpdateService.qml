...
QtObject {
    ...
    // ── Paths ──────────────────────────────────────────────────────────────
    readonly property string _dir:        Quickshell.env("HOME") + "/.local/src/Ghost"
    readonly property string _cfgPath:    Quickshell.env("HOME") + "/.config/Ghost/src/user_data/update_prefs.json"
    ...
    property var _stashPullProc: Process {
        command: ["bash", "-c",
            ...
            "git -C '" + root._dir + "' stash push -m 'ghost-pre-update' 2>/dev/null || true; " +

...
//   system only → GhostMixer null sink + loopback from sink.monitor
//   both        → GhostMixer null sink + loopback from sink.monitor
...
QtObject {
    ...
    property bool   _usingNullSink: false // true while GhostMixer is active
    ...
    Component.onCompleted: {
        var path = Quickshell.env("HOME") + "/.config/Ghost/src/user_data/screenrec.json"
    ...
    // system only: create GhostMixer null sink, route sink.monitor into it.
    // both:        create GhostMixer null sink, route sink.monitor AND
    ...
    function _resolveAudio() {
        ...
        // System audio (alone or combined with mic) — needs GhostMixer
        ...
        var script =
            ...
            "pactl load-module module-null-sink sink_name=GhostMixer >/dev/null; " +
            "sleep 0.3; " +
            // Route system audio (default sink monitor) into GhostMixer
            "pactl load-module module-loopback " +
            "sink=GhostMixer source=$(pactl get-default-sink).monitor >/dev/null"

        if (root.audioMic && root.audioSystem) {
            // Also route mic into GhostMixer
            script += "; pactl load-module module-loopback " +
                      "sink=GhostMixer source=$(pactl get-default-source) >/dev/null"
        }

        // Emit the recording device name as the final stdout line
        script += "; printf 'GhostMixer.monitor\\n'"
    ...
    function _startCavaWithSource(src) {
        ...
        _cavaRecProc.command = [
            "bash", "-c",
            "mkdir -p /tmp/ghost && printf '%s\\n' '" +
            config.replace(/'/g, "'\\''") +
            "' > /tmp/ghost/cava_rec.ini && " +
            "exec cava -p /tmp/ghost/cava_rec.ini 2>/dev/null"
        ]
